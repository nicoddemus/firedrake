...
def p1_coarse_fine_map(Vc, Vf, np.ndarray[PetscInt, ndim=2, mode="c"] c2f_cells):
    ...
    cdef:
        ...
        np.ndarray[PetscInt, ndim=1, mode="c"] orig_c2f
    ...
    map_vals = np.empty((Vc.mesh().cell_set.size, ndof), dtype=np.int32)
    map_vals[:, :] = -1
    ...
    orig_c2f = coarse_mesh._plex.createCoarsePointIS().indices
